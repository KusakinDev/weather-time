...
func GetWeatherByName(c *gin.Context, city string) {

	logger.Infoln("Find by name:", city)

	weatherFront, isFind := cache.Cache.FindSTRING(city)
	if isFind {
	...
	logger.Infoln("Response to front:", weatherFront)
	cache.Cache.InsertSTRING(weatherFront.Name, weatherFront)
